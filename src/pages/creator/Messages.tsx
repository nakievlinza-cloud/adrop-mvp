...
export function CreatorMessages() {
  ...
  useEffect(() => {
    ...
    const chatsQuery = query(collection(db, "chats"), where("creatorId", "==", user.uid));
    const unsubscribe = onSnapshot(chatsQuery, async (snapshot) => {
      const chatsData = await Promise.all(
        snapshot.docs.map(async (docSnapshot) => {
          const data = docSnapshot.data();

          let customerName = "Заказчик";
          let customerAvatar = getAvatarSrc(null, customerName, "customer");

          try {
            const customerDoc = await getDoc(doc(db, "users", data.customerId));
            if (customerDoc.exists()) {
              customerName = customerDoc.data().name || customerName;
              customerAvatar = getAvatarSrc(
                customerDoc.data().avatarUrl || customerDoc.data().avatar,
                customerName,
                "customer",
              );
            }
  ...
  const handleOpenChat = (chatId: string) => {
    ...
    setShowMenu(false);
    setSendError("");
    window.history.replaceState({}, "", `/creator/messages?chat=${chatId}`);
  };
        ...
        <div className={`w-full lg:w-[360px] border-r border-white/5 bg-background/50 flex-col ${showChatPanel ? "hidden lg:flex" : "flex"}`}>
          <div className="p-5 border-b border-white/5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-11 bg-card border border-white/10 rounded-xl pl-10 pr-4 focus:outline-none focus:border-ugc-primary transition-colors text-sm"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredChats.map((chat) => (
              <button
                ...
                className={`w-full p-4 flex items-center gap-4 hover:bg-white/5 transition-colors text-left border-b border-white/5 ${
                  activeChatId === chat.id ? "bg-white/5 border-l-2 border-l-ugc-primary" : "border-l-2 border-l-transparent"
                }`}
              >
                <div className="relative shrink-0">
                  <img src={chat.avatar} alt={chat.name} className="w-12 h-12 rounded-full object-cover" />
                  {chat.unread > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 bg-ugc-primary rounded-full flex items-center justify-center text-[10px] font-bold text-white shadow-lg">
                      {chat.unread}
                    </span>
                  )}
                </div>
            ...
            {!filteredChats.length && (
              <div className="p-6 text-sm text-foreground/50">Чатов пока нет. Как только заказчик напишет вам, диалог появится здесь.</div>
            )}
          </div>
        </div>
                  ...
                  if (isSystem) {
                    return (
                      <div key={message.id} className="flex justify-center">
                        <div className="inline-flex max-w-full items-center gap-2 px-4 py-2 bg-ugc-primary/10 border border-ugc-primary/20 rounded-full text-sm text-center">
                          <AppIcon icon={CheckmarkCircle02Icon} className="text-ugc-primary shrink-0" size={16} />
                          <span className="text-ugc-primary font-medium break-words">{message.text}</span>
                        </div>
                      </div>
                    );
                  }
                    ...
                    <div key={message.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`w-full max-w-[88%] sm:max-w-[70%] p-4 ${
                          isMine
                            ? "bg-gradient-creator rounded-2xl rounded-tr-none text-white shadow-lg shadow-ugc-primary/25"
                ...
                <form onSubmit={handleSendMessage} className="flex items-end gap-2 sm:gap-3">
                  ...
                  <div className="flex-1">
                    <textarea
                      ...
                      rows={1}
                      className="w-full min-h-[48px] max-h-32 resize-none bg-card border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-ugc-primary transition-colors"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-ugc-primary flex items-center justify-center hover:bg-ugc-primary/90 transition-colors text-white shadow-lg shadow-ugc-primary/20 disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                  >
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full h-12 bg-card border border-white/10 rounded-xl px-4 focus:outline-none focus:border-ugc-primary transition-colors text-sm"
                  >
